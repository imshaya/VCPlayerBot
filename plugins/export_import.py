    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        if m.reply_to_message is not None and m.reply_to_message.document:
            if m.reply_to_message.document.file_name != "PlayList.json":
                k=await m.reply("فایله پلی لیست نامعتبره")
                await delete_messages([m, k])
                return
            myplaylist=await m.reply_to_message.download()
            status=await m.reply("درحاله گرفتنه اطلاعاته پلی لیست")
            n=await import_play_list(myplaylist)
            if not n:
                await status.edit("وارد کردنه اطلاعات پلی لیست با مشکل مواجه شد")
